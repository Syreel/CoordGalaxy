...
SELECTED_DATASET = "russia2"
RAW_TWEETS_FILE = "russia2.csv"
NORMALIZED_TWEETS_FILE = "normalized_tweets.csv"


def preprocess_russia2_input(config: PipelineConfig, paths: DatasetPaths, ch: Checkpoint, im: InputManager) -> None:
    """
    Normalize Russia2 source files and extract Information Operations co-action artifacts.

    :param config: Pipeline configuration.
    :param paths: Russia2 path bundle.
    :param ch: Checkpoint instance.
    :param im: InputManager configured for the Russia2 dataset.
...
def run_user_selection(config: PipelineConfig, lm: LogManager) -> None:
    ...
    if config.user_fraction is None:
        lm.printl("main_russia2. user_fraction=None, skipping user filtering and using all users.")
        return
...
def compute_similarity_edges(config: PipelineConfig) -> None:
    """
    Compute similarity edge lists for configured Russia2 co-actions.
...
def run_pipeline(config: PipelineConfig) -> None:
    """
    Execute the Russia2 coordinated-behavior pipeline.
    ...
    preprocess_russia2_input(config, paths, ch, im)
